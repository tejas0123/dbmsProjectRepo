...
exports.viewcar=(req,res,next)=>{
    Product.getById(req.query.id).then(result=>{
       Product.getByname(req.query.name).then(specs=>{
        res.render('view-car',{product:result[0][0],specs:specs[0][0],user:req.session.user});
       }).catch(err=>console.log(err));
    }).catch(err=>console.log(err));
}
...
exports.add_User=(req,res,next)=>{
    if(req.body.username=="" || req.body.firstname=="" || req.body.lastname=="" || req.body.password=="" || req.body.confirm=="" || req.body.address=="")
        {
...
exports.brands=(req, res)=>{
    ...
    Product.brands(req.query.brand).then(result=>{
        ...
        res.render('brands',{results:result[0], user:req.session.user});
    }).catch(err=>{
        console.log(err);
    });
}
